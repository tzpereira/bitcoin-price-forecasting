...
from backend.services.forecasts_storage import save_forecast_run, create_lock, remove_lock
...
def run_linear_regression_forecast(horizon=365, window_size=1095, progress_callback=None):
    ...
    try:
        ...
        save_forecast_run(model_name, run_date, horizon, [
            {"target_date": row["Datetime"][:10], "prediction": row["prediction"]} for row in future_rows
        ], params=None)
        return future_rows
    finally:
        remove_lock(model_name)

def run_xgboost_forecast(horizon=365, window_size=1095, progress_callback=None):
    """
    ...
    try:
        ...
        serializable_params = {"window_size": window_size, **default_params}
        save_forecast_run(model_name, run_date, horizon, [
            {"target_date": row["Datetime"][:10], "prediction": row["prediction"]} for row in future_rows
...
def run_sarimax_forecast(horizon=365):
    """
    ...
    try:
        ...
        try:
            ...
            pass
        save_forecast_run(model_name, run_date, horizon, future_rows, params=None)
